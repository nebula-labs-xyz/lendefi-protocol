...
import "../contracts/lender/lib/UniswapTickMath.sol";
...
contract OracleForkTest is BasicDeploy {
    ...
    function setUp() public {
        ...
        mainnetFork = vm.createFork("mainnet", 18_500_000);
    ...
    function testRevert_PoolLiquidityLimitReached() public {
        ...
        // Create a position
        LendefiInstance.createPosition(WETH, false);
        uint256 positionId = LendefiInstance.getUserPositionsCount(testUser) - 1;
        ...
        vm.startPrank(testUser);
        IERC20(WETH).approve(address(LendefiInstance), supplyAmount);
        vm.expectRevert(IPROTOCOL.PoolLiquidityLimitReached.selector);
        LendefiInstance.supplyCollateral(WETH, supplyAmount, positionId);
        vm.stopPrank();
    ...
    function testRevert_AssetLiquidityLimitReached() public {
        ...
        // Create a position
        LendefiInstance.createPosition(WETH, false); // false = cross-collateral position
        uint256 positionId = LendefiInstance.getUserPositionsCount(testUser) - 1;
        console2.log("Created position ID:", positionId);
        ...
        // Supply within limit
        vm.startPrank(testUser);
        IERC20(WETH).approve(address(LendefiInstance), 0.5 ether);
        LendefiInstance.supplyCollateral(WETH, 0.5 ether, positionId);
        console2.log("Supplied 0.5 WETH");

        // Try to exceed the limit
        IERC20(WETH).approve(address(LendefiInstance), 1 ether);
        vm.expectRevert(IPROTOCOL.AssetCapacityReached.selector);
        LendefiInstance.supplyCollateral(WETH, 1 ether, positionId);
        vm.stopPrank();
    ...
    function test_getAnyPoolTokenPriceInUSD_ETHUSDC() public {
        ...
        uint256 ethPriceInUSD = UniswapTickMath.getAnyPoolTokenPriceInUSD(ethUsdcPool, WETH, ethUsdcPool, 1800);
        ...
        assertTrue(ethPriceInUSD > 1700 * 1e6, "ETH price should be greater than $1000");
        assertTrue(ethPriceInUSD < 2000 * 1e6, "ETH price should be less than $5000");
    }

    function test_getAnyPoolTokenPriceInUSD_WBTCETH() public {
        // WBTC/ETH pool address
        address wbtcEthPool = 0xCBCdF9626bC03E24f779434178A73a0B4bad62eD; // WBTC/ETH pool
        address ethUsdcPool = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640; // ETH/USDC pool

        // Call the getAnyPoolTokenPriceInUSD function
        uint256 wbtcPriceInUSD = UniswapTickMath.getAnyPoolTokenPriceInUSD(wbtcEthPool, WBTC, ethUsdcPool, 1800);

        // Log the WBTC price in USD
        console2.log("WBTC price in USD (from WBTC/ETH pool):", wbtcPriceInUSD);

        // Assert that the price is within a reasonable range (e.g., $30,000 to $40,000)
        assertTrue(wbtcPriceInUSD > 33000 * 1e6, "WBTC price should be greater than $30,000");
        assertTrue(wbtcPriceInUSD < 35000 * 1e6, "WBTC price should be less than $40,000");
    }

    function test_getAnyPoolTokenPriceInUSD_LINKETH() public {
        // LINK/ETH pool address
        address linkEthPool = 0xa6Cc3C2531FdaA6Ae1A3CA84c2855806728693e8; // LINK/ETH pool
        address ethUsdcPool = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640; // ETH/USDC pool

        // Call the getAnyPoolTokenPriceInUSD function
        uint256 linkPriceInUSD = UniswapTickMath.getAnyPoolTokenPriceInUSD(linkEthPool, LINK, ethUsdcPool, 1800);

        // Log the LINK price in USD
        console2.log("LINK price in USD (from LINK/ETH pool):", linkPriceInUSD);

        // Assert that the price is within a reasonable range (e.g., $5 to $50)
        assertTrue(linkPriceInUSD > 9 * 1e6, "LINK price should be greater than $5");
        assertTrue(linkPriceInUSD < 12 * 1e6, "LINK price should be less than $50");
    }

    function test_getAnyPoolTokenPriceInUSD_WBTCUSDC() public {
        // WBTC/USDC pool address (replace with the actual pool address)
        address wbtcUsdcPool = WBTC_USDC_POOL; // Example WBTC/USDC pool address
        address wbtcToken = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599; // WBTC token address
        address ethUsdcPool = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640; // ETH/USDC pool address

        // Call the function to get the WBTC price in USD
        uint256 wbtcPriceInUSD = UniswapTickMath.getAnyPoolTokenPriceInUSD(wbtcUsdcPool, wbtcToken, ethUsdcPool, 1800);

        // Log the WBTC price in USD
        console2.log("WBTC price in USD (from WBTC/USDC pool):", wbtcPriceInUSD);

        // Assert that the price is within a reasonable range (e.g., $20,000 to $50,000)
        assertTrue(wbtcPriceInUSD > 34000 * 1e6, "WBTC price should be greater than $20,000");
        assertTrue(wbtcPriceInUSD < 35000 * 1e6, "WBTC price should be less than $50,000");
    }
}
