...
contract AssetsInitializeTest is BasicDeploy {
    ...
    function setUp() public {
        ...
        oracleAddr = address(oracleInstance);

        // Create initialization data
        initData = abi.encodeCall(LendefiAssets.initialize, (timelockAddr, oracleAddr, guardian));
    }
    ...
    function test_ZeroAddressReverts() public {
        ...
        assetsModule.initialize(address(0), oracleAddr, guardian);

        // Test with zero address for oracle
        vm.expectRevert(abi.encodeWithSignature("ZeroAddressNotAllowed()"));
        assetsModule.initialize(timelockAddr, address(0), guardian);

        // Test with zero address for guardian
        vm.expectRevert(abi.encodeWithSignature("ZeroAddressNotAllowed()"));
        assetsModule.initialize(timelockAddr, oracleAddr, address(0));
    }
    ...
    function test_PreventReinitialization() public {
        ...
        assetsContract.initialize(timelockAddr, oracleAddr, guardian);
    }
    ...
    function test_TierParameterPrecision() public {
        ...
        assertEq(
            assetsContract.getTierLiquidationFee(ILendefiAssets.CollateralTier.ISOLATED),
            0.04e6,
            "ISOLATED liquidation fee should be precisely 0.04e6"
        );

        assertEq(
            assetsContract.getTierJumpRate(ILendefiAssets.CollateralTier.STABLE),
            0.05e6,
    ...
    function test_PauseStateAfterInit() public {
        ...
        bytes memory localInitData = abi.encodeCall(LendefiAssets.initialize, (timelockAddr, oracleAddr, guardian));
        ...
        oracleInstance.addOracle(address(wethInstance), mockPriceFeed, 8);
        oracleInstance.setPrimaryOracle(address(wethInstance), mockPriceFeed);
    ...
    function test_InitializeWithDifferentGuardian() public {
        ...
        bytes memory customData = abi.encodeCall(LendefiAssets.initialize, (timelockAddr, oracleAddr, newGuardian));
