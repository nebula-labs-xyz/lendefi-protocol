...
contract ExitPositionTest is BasicDeploy {
    ...
    function _setupAssets() internal {
        ...
        // UPDATED: Use assetsInstance for configuration
    ...
    function test_ExitIsolatedPosition() public {
        ...
        // Check collateral is returned - UPDATED: use getCollateralAmount instead of getUserCollateralAmount
        uint256 remainingCollateral = LendefiInstance.getCollateralAmount(bob, positionId, address(rwaToken));
    ...
    function test_ExitPositionMultipleAssets() public {
        ...
        // Check no collateral remains - UPDATED: use getCollateralAmount
        uint256 remainingWeth = LendefiInstance.getCollateralAmount(bob, positionId, address(wethInstance));
        uint256 remainingStable = LendefiInstance.getCollateralAmount(bob, positionId, address(stableToken));
    ...
    function test_ExitPositionInsufficientUSDC() public {
        ...
        // Try to exit position without enough USDC - UPDATED: use string error code
        vm.expectRevert(); // The specific transfer error varies by implementation
        LendefiInstance.exitPosition(positionId);
        vm.stopPrank();
    ...
    function test_ExitAndReusePosition() public {
        ...
        // Cannot supply collateral to closed position - UPDATED: use string error code
    ...
    function test_StorageSlotsAfterExit() public {
        ...
        // Ensure collateral storage is properly cleared - UPDATED: use getCollateralAmount
        uint256 wethAmount = LendefiInstance.getCollateralAmount(bob, positionId, address(wethInstance));
        uint256 stableAmount = LendefiInstance.getCollateralAmount(bob, positionId, address(stableToken));
    ...
    function test_PositionSummaryAfterExit() public {
        ...
        uint256 totalCollateralValue = LendefiInstance.calculateCollateralValue(bob, positionId);
        uint256 currentDebt = LendefiInstance.calculateDebtWithInterest(bob, positionId);
        uint256 availableCredit = LendefiInstance.calculateCreditLimit(bob, positionId);
        ...
        assertFalse(position.isIsolated, "Position should not be isolated");
        assertEq(uint256(position.status), uint256(IPROTOCOL.PositionStatus.CLOSED), "Status should be CLOSED");
    }
    ...
    function testFuzz_ExitPositionsWithVaryingDebt(uint256 debtPct) public {
        ...
        assertEq(position.debtAmount, 0, "Position debt should be cleared");
        uint256 remainingCollateral = LendefiInstance.getCollateralAmount(bob, positionId, address(wethInstance));
