...
contract UpdateProtocolConfigTest is BasicDeploy {
    ...
    function testRevert_ResetProtocolConfig_AccessControl() public {
        // Regular user should not be able to reset
        vm.startPrank(alice);
        ...
        LendefiInstance.resetProtocolConfig();
        vm.stopPrank();

        // Manager should be able to reset
        vm.prank(address(timelockInstance));
        LendefiInstance.resetProtocolConfig();
    }
    ...
    function test_ResetProtocolConfig_StateChange() public {
        // First update to non-default values
        IPROTOCOL.ProtocolConfig memory newConfig = _createValidConfig();
        vm.prank(address(timelockInstance));
        LendefiInstance.loadProtocolConfig(newConfig);
        ...
        IPROTOCOL.ProtocolConfig memory resetConfig = LendefiInstance.getConfig();
        assertEq(resetConfig.profitTargetRate, DEFAULT_BASE_PROFIT_TARGET);
        assertEq(resetConfig.borrowRate, DEFAULT_BASE_BORROW_RATE);
        assertEq(resetConfig.rewardAmount, DEFAULT_TARGET_REWARD);
        assertEq(resetConfig.rewardInterval, DEFAULT_REWARD_INTERVAL);
        assertEq(resetConfig.rewardableSupply, DEFAULT_REWARDABLE_SUPPLY);
        assertEq(resetConfig.liquidatorThreshold, DEFAULT_LIQUIDATOR_THRESHOLD);
        assertEq(resetConfig.flashLoanFee, DEFAULT_FLASH_LOAN_FEE);
    }
