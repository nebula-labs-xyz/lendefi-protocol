...
contract LendefiAssetsTest is BasicDeploy {
    ...
    function setUp() public {
        ...
        oracleInstance.addOracle(address(linkInstance), address(linkOracle), 8);
        oracleInstance.setPrimaryOracle(address(linkInstance), address(linkOracle));

        oracleInstance.addOracle(address(uniInstance), address(uniOracle), 8);
        oracleInstance.setPrimaryOracle(address(uniInstance), address(uniOracle));

        oracleInstance.addOracle(address(usdcInstance), address(stableOracle), 8);
        oracleInstance.setPrimaryOracle(address(usdcInstance), address(stableOracle));
        vm.stopPrank();
    ...
    function test_UpdateAssetConfig() public {
        ...
        emit ILendefiAssets.UpdateAssetConfig(address(wethInstance));
        assetsInstance.updateAssetConfig(
            address(wethInstance),
            address(wethOracle),
            8, // Oracle decimals
        ...
        assertEq(uint256(assetInfo.tier), uint256(ILendefiAssets.CollateralTier.CROSS_A));
        assertEq(assetInfo.isolationDebtCap, 0);

        // Update USDC with a different configuration
        vm.expectEmit(true, false, false, false);
        emit ILendefiAssets.UpdateAssetConfig(address(usdcInstance));
        assetsInstance.updateAssetConfig(
            address(usdcInstance),
            address(stableOracle),
            8, // Oracle decimals
        ...
        ILendefiAssets.Asset memory usdcInfo = assetsInstance.getAssetInfo(address(usdcInstance));
    ...
    function test_UpdateAssetTier() public {
        ...
        ILendefiAssets.Asset memory assetInfo = assetsInstance.getAssetInfo(address(wethInstance));
        assertEq(uint256(assetInfo.tier), uint256(ILendefiAssets.CollateralTier.CROSS_B));
    }

    function testRevert_UpdateAssetTier_AssetNotListed() public {
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.AssetNotListed.selector, address(0xC0FFEE)));
        assetsInstance.updateAssetTier(address(0xC0FFEE), ILendefiAssets.CollateralTier.CROSS_B);
    }

    // ------ Oracle Tests ------

    function test_UpdateOracleModule() public {
        address newOracle = address(0xA);

        vm.prank(address(timelockInstance));

        assetsInstance.updateOracleModule(newOracle);

        assertEq(address(assetsInstance.oracleModule()), newOracle);
    }
    ...
    function test_UpdateTierParameters() public {
        ...
        assetsInstance.updateTierParameters(ILendefiAssets.CollateralTier.CROSS_A, newJumpRate, newLiquidationFee);

        // Verify updated parameters
        assertEq(assetsInstance.getTierJumpRate(ILendefiAssets.CollateralTier.CROSS_A), newJumpRate);
        assertEq(assetsInstance.getTierLiquidationFee(ILendefiAssets.CollateralTier.CROSS_A), newLiquidationFee);
        assertEq(assetsInstance.getLiquidationFee(ILendefiAssets.CollateralTier.CROSS_A), newLiquidationFee);
    }
    ...
    function testRevert_UpdateTierParameters_RateTooHigh() public {
        ...
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.RateTooHigh.selector, excessiveJumpRate, 0.25e6));
        assetsInstance.updateTierParameters(ILendefiAssets.CollateralTier.CROSS_A, excessiveJumpRate, 0.03e6);
    }
    ...
    function testRevert_UpdateTierParameters_FeeTooHigh() public {
        ...
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.FeeTooHigh.selector, excessiveFee, 0.1e6));
        assetsInstance.updateTierParameters(ILendefiAssets.CollateralTier.CROSS_A, 0.1e6, excessiveFee);
    }
    ...
    function testRevert_AssetNotListed_GetAssetInfo() public {
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.AssetNotListed.selector, address(0xDEAD)));
    ...
    function test_CollateralTierParameters() public {
        ...
        tiers[1] = ILendefiAssets.CollateralTier.CROSS_A;
        tiers[2] = ILendefiAssets.CollateralTier.CROSS_B;
    ...
    function test_AddNewAsset() public {
        ...
        vm.startPrank(address(timelockInstance));
        oracleInstance.addOracle(address(newToken), address(wethOracle), 8);
        oracleInstance.setPrimaryOracle(address(newToken), address(wethOracle));

        // Add the new asset
        vm.expectEmit(true, false, false, false);
        emit ILendefiAssets.UpdateAssetConfig(address(newToken));
        assetsInstance.updateAssetConfig(
            address(newToken),
    ...
    function testRevert_ThresholdsTooHigh() public {
        ...
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.InvalidLiquidationThreshold.selector, 991));
        assetsInstance.updateAssetConfig(
            address(wethInstance),
            address(wethOracle),
            8,
        ...
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.InvalidBorrowThreshold.selector, 891));
        assetsInstance.updateAssetConfig(
            address(wethInstance),
            address(wethOracle),
            8,
    ...
    function testRevert_InvalidThresholds() public {
        ...
        vm.prank(address(timelockInstance));
        vm.expectRevert(abi.encodeWithSelector(ILendefiAssets.InvalidBorrowThreshold.selector, 850));
        assetsInstance.updateAssetConfig(
            address(wethInstance),
            address(wethOracle),
            8,
    ...
    function test_InitializeSuccess() public {
        ...
        address oracleAddr = address(oracleInstance);
        ...
        // Check oracle assignment
        assertEq(address(assetsContract.oracleModule()), oracleAddr, "Oracle address incorrect");
