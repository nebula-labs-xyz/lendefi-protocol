...
contract CalculateCollateralValueTest is BasicDeploy {
    ...
    function _setupAssets() internal {
        ...
        assetsInstance.addOracle(address(wethInstance), address(ethOracle), 8);
        assetsInstance.setPrimaryOracle(address(wethInstance), address(ethOracle));

        assetsInstance.addOracle(address(rwaToken), address(rwaOracle), 8);
        assetsInstance.setPrimaryOracle(address(rwaToken), address(rwaOracle));

        assetsInstance.addOracle(address(stableToken), address(stableOracle), 8);
